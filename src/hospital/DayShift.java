...
public class DayShift extends Nurse implements Hospital {
    ...
    public static List<String> returnAll(Connection c) throws SQLException {
        ...
        String query = "SELECT * FROM Shift WHERE shift = 'day' ORDER BY nurse_id ASC";
    ...
    public void delete(Connection c) throws SQLException {
        setStatement(c.createStatement());
        String query = "DELETE FROM Shift WHERE nurse_id = " + this.getId() + " AND shift = 'day'";

        PreparedStatement preparedStmt = c.prepareStatement(query);
        preparedStmt.execute();

        System.out.println("Nurse " + this.getId() + " with Day Shift deleted from table 'Patient'");
    }
